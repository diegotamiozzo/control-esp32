...
// =================================================================
// SISTEMA DE CONTROLE INDUSTRIAL - VERSÃO CORRIGIDA
// Entradas: I1-I5 | Saídas: Q1-Q7 | Sensor: DHT11
// =================================================================
...
// =================================================================
// PINAGEM CORRIGIDA - SEM STRAPPING PINS
// =================================================================
...
// --- ENTRADAS DIGITAIS (I1-I5) - TODOS SEGUROS ---
...
// --- SAÍDAS (Q1-Q7) - EVITANDO STRAPPING PINS ---
...
void controlLogic() {
  ...
  if (inputs.i1_habilitacao) {
    ...
    if (alarme_enabled && (temp_out || umid_out)) {
      ...
      unsigned long cycle_time = (time_alarme_on + time_alarme_off) * 1000;
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (doc.containsKey("sp_temp")) { sp_temp = doc["sp_temp"].as<float>(); settings_changed = true; }
  if (doc.containsKey("sp_umid")) { sp_umid = doc["sp_umid"].as<float>(); settings_changed = true; }
  if (doc.containsKey("hist_temp")) { hist_temp = doc["hist_temp"].as<float>(); settings_changed = true; }
  if (doc.containsKey("hist_umid")) { hist_umid = doc["hist_umid"].as<float>(); settings_changed = true; }
  ...
  if (doc.containsKey("manual_mode")) {
    ...
    Serial.printf("🎛️ Modo Manual: %s\n", manual_mode ? "ATIVADO" : "DESATIVADO");
  }
...
void setup() {
  ...
  if (!wm.autoConnect("ESP32_INDUSTRIAL", "senha123")) {
    Serial.println("❌ WiFi falhou. Reiniciando...");
    ESP.restart();
  }
  ...
  Serial.printf("📱 MAC: %s\n", client_id);
  Serial.printf("📡 Tópico telemetria: %s\n", topic_telemetry);
